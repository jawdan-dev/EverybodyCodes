...
#	define NODE_MAX 1000
	...
	for (int i = 0; i < width && entry.x == -1; i++) {
		if (map[i][0] == '.') entry = (Point) { .x = i, .y = 0 };
		else if (map[i][height - 1] == '.') entry = (Point) { .x = i, .y = height - 1 };
	}
	for (int i = 0; i < height && entry.x == -1; i++) {
		if (map[0][i] == '.') entry = (Point) { .x = 0, .y = i };
		else if (map[width - 1][i] == '.') entry = (Point) { .x = width - 1, .y = i };
	}
